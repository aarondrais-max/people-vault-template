# People Team Knowledge Vault — Agent Instructions

This is a local People/recruiting knowledge vault maintained by an AI agent. When operating in this project, follow all conventions below exactly.
...
> **Before setup:** this file ships as the recruiter-flavoured default. `skills/vault-setup/SKILL.md` replaces it with the variant for your role from `roles/` (recruiter, hrbp, comp, generic).
...
├── roles/                ← role variants of this file (removed after setup)
└── skills/               ← one directory per skill, each holding a SKILL.md
    ├── vault-boot/SKILL.md
    ├── vault-orchestrator/SKILL.md
    └── ...
```
